...
def diff_exp(adata_subset, TOI):
    ...
    counts = pd.DataFrame(adata_subset.X, columns=adata_subset.var_names, index=adata_subset.obs_names)
    ...
    dds = DeseqDataSet(counts=counts, metadata=metadata, refit_cooks=True)

    dds.deseq2()
    ds = DeseqStats(dds, contrast=["condition", TOI, "DMSO-TF"])  
    ds.summary()
